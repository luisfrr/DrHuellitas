...
namespace DrHuellitas.DAO
{
    public class AgendaDAO
    {
        ...
        public List<CitasBO> GetEventsUser(int id)
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT c.id,c.titulo,c.color,c.descripcion,c.fechaInicio, c.fechaFin, (SELECT nombreComercial FROM Comercio WHERE id = (SELECT idsucursal FROM UsuarioComercio WHERE idempresa = c.idComercio)) AS nombrecomercio FROM Citas c WHERE c.idUsuario=@id");
            cmd.Parameters.Add("@id", SqlDbType.Int).Value = id;
